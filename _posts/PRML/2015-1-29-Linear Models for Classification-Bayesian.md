...
<img src="http://chrispher.github.com/images/prml/ch4_bayesian_laplace.jpg" height="100%" width="100%">
...
这里A是hessian矩阵。上式第一项是在最优参数下的对数概率估计，剩余的三项是奥卡姆因子(Occam factor)，用于惩罚模型的复杂度。如果假设参数的先验概率是高斯分布，而且hessian矩阵是满秩的，那么我们可以近似上式为$$\ln p(D) \cong p(D \mid \theta_{MAP}) - \frac{1}{2}M \n N$$。这里N是样本数，M是参数个数，该式子也称之为贝叶斯信息准则(BIC, Bayesian Information Criterion)。与第一章的AIC($$\ln p(D \mid w_{ML}) - M$$)相比,对模型的复杂度惩罚更厉害。
...
近似的高斯分布形式就是q(w) = N(w \mid w_{MAP}, S_N)
