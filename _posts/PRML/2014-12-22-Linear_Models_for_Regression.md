...
我们深入的理解一下边缘似然。假设只有一个参数w，它的先验分布是在$$w_{MAP}$$快速达到峰值,宽度为$$\Delta w_{posterior}$$的分布（见下图左侧图）。我们继续假设后验分布是宽度为$$\Delta w_{prior}$$的均匀分布，那么$$p(w) = 1 / \Delta w_{prior}$$，那么我们有（为方便显示，忽略$$M_i$$）：
