...
在博客的首页，通常只显示了文章的一部分内容，这是怎么实现的呢？一般而言，我们写作时，会在开头写一下整篇文章的概述，之后在增加一行
{% highlight python %}
<!-- more -->
{% endhighlight %}

那么一般显示的时候就只显示标记之前的文本，只有点击阅读进入这篇文章才能看到全部内容。当然，这里也是跟html语法相关的，在index.html里的
{% highlight python %}
...
####4.4代码高亮
代码高亮语法是：在代码开始的地方标注 
大括号{之后跟一个%，再加highlight python,之后再加大括号{之后跟一个%
在代码结束的地方标注：大括号{之后跟个%，再加 endhighlight之后再加大括号{之后跟一个%。
那么中间的代码就是使用了Python的代码高亮风格，具体大家可以看博客的源文件！